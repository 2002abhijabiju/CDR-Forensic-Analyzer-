...
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, func, select, text, delete
...
# ---------- location engine import (fixed) ----------
try:
    from location_engine import estimate_location
    LOCATION_ENGINE_AVAILABLE = True
except ImportError:
    print("⚠️ location_engine.py not found. Geolocation features will be disabled.")
...
async def upload_cdr(case_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    # Column detection (same as original, kept intact)
    def detect_columns(df, sample_size=10):
        ...
        name_hints = {
            ...
            'number': ['number', 'called_number', 'phone', 'mobile', 'b_number'],
            'duration': ['duration', 'duration_sec', 'duration_s', 'call_duration', 'talktime', 'used usage'],
    ...
    if 'datetime' not in roles and ('date' not in roles or 'time' not in roles):
        raise HTTPException(400, f"Could not identify date/time columns. Need combined datetime or separate date+time. Found columns: {list(df.columns)}")
...
# ---------- Records & Analysis Endpoints (unchanged) ----------
...
async def relationship_mapping(case_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not case:
        raise HTTPException(404, "Case not found")
    result = await db.execute(select(CDRRecord.called_number, CDRRecord.call_type, CDRRecord.duration_seconds, CDRRecord.call_datetime).where(CDRRecord.case_id == case_id))
    rows = result.all()
    if not rows:
    ...
    entities = [{"id": case.subscriber_number, "type": "Subscriber", "label": f"📱 {case.subscriber_number}", "properties": {"case_number": case.case_number, "total_calls": sum(c["voice_count"] + c["sms_count"] for c in contacts_data.values())}}]
    for number, data in contacts_data.items():
        entities.append({"id": number, "type": "Contact", "label": f"📞 {number}", "properties": {"voice_calls": data["voice_count"], "sms": data["sms_count"], "total_duration_sec": data["total_duration"], "first_call": data["first_call"].isoformat() if data["first_call"] else None, "last_call": data["last_call"].isoformat() if data["last_call"] else None}})
    relationships = []
    for number, data in contacts_data.items():
        link_type = "CALLED"
        ...
        relationships.append({"source": case.subscriber_number, "target": number, "type": link_type, "weight": weight, "properties": {"voice_calls": data["voice_count"], "sms": data["sms_count"], "total_duration": data["total_duration"], "first_seen": data["first_call"].isoformat() if data["first_call"] else None, "last_seen": data["last_call"].isoformat() if data["last_call"] else None}})
...
async def forensic_anomaly_dashboard(case_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not records:
        return {"summary": {"total_anomalies": 0, "night_activity_count": 0, "spike_events": 0, "new_contacts": 0}, "top_suspicious": [], "spike_data": [], "night_heatmap": [], "event_log": []}
    df = pd.DataFrame([{"called": r.called_number, "duration": r.duration_seconds, "hour": r.call_datetime.hour, "date": r.call_datetime.date(), "datetime": r.call_datetime} for r in records])
    ...
    if df["date"].nunique() >= 2:
        ...
        for _, row in daily_counts.iterrows():
            ...
            if is_spike:
                spike_events += 1
            spike_data.append({"date": row["date"].isoformat(), "count": int(row["count"]), "baseline": round(baseline, 1), "is_spike": is_spike})
    else:
        spike_data = [{"date": df["date"].iloc[0].isoformat(), "count": len(df), "baseline": len(df), "is_spike": False}]
    new_contacts = []
    contact_stats = []
    total_calls = len(df)
    for contact in df["called"].unique():
        ...
        contact_stats.append({"number": contact, "score": round(score, 1), "risk": risk, "flags": flags})
    top_suspicious = sorted(contact_stats, key=lambda x: x["score"], reverse=True)[:10]
    heatmap = []
    for _, row in night_calls.iterrows():
        heatmap.append({"day": row["date"].isoformat(), "hour": row["hour"], "count": 1})
    agg = defaultdict(int)
    for h in heatmap:
        agg[(h["day"], h["hour"])] += 1
    ...
    for _, row in night_calls.head(50).iterrows():
        event_log.append({"timestamp": row["datetime"].isoformat(), "number": row["called"], "type": "night_activity", "description": f"Call at {row['datetime'].strftime('%H:%M')} (unusual hour)"})
    for _, row in long_calls.head(20).iterrows():
        event_log.append({"timestamp": row["datetime"].isoformat(), "number": row["called"], "type": "long_call", "description": f"Long call: {row['duration']} seconds (>5 min)"})
    for spike in spike_data:
        if spike.get("is_spike", False):
            event_log.append({"timestamp": spike["date"], "number": "—", "type": "spike", "description": f"Call volume spike: {spike['count']} vs baseline {spike['baseline']}"})
    anomalies_found = (night_activity_count > 0) or (long_call_count > 5) or (spike_events > 0) or (len([s for s in top_suspicious if s["risk"] == "HIGH"]) > 0)
    summary = {"total_anomalies": len(event_log), "night_activity_count": night_activity_count, "spike_events": spike_events, "new_contacts": len(new_contacts)}
    if not anomalies_found:
        return {"summary": {"total_anomalies": 0, "night_activity_count": 0, "spike_events": 0, "new_contacts": 0}, "top_suspicious": [], "spike_data": spike_data, "night_heatmap": [], "event_log": []}
    return {"summary": summary, "top_suspicious": top_suspicious, "spike_data": spike_data, "night_heatmap": night_heatmap[:20], "event_log": event_log[:50]}

# ---------- NEW: Location Engine Integration ----------
...
def load_settings():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            return json.load(f)
    return {"unwired_api_token": "", "opencellid_api_token": "", "google_api_key": ""}
...
def save_token(data: dict):
    ...
    if "unwired_token" in data:
        settings["unwired_api_token"] = data["unwired_token"]
    if "opencellid_token" in data:
        settings["opencellid_api_token"] = data["opencellid_token"]
    if "google_key" in data:
        settings["google_api_key"] = data["google_key"]
    save_settings(settings)
    return {"status": "ok"}

@app.get("/api/load_token")
def load_token():
    settings = load_settings()
    return {"unwired_token": settings.get("unwired_api_token", ""), "opencellid_token": settings.get("opencellid_api_token", ""), "google_key": settings.get("google_api_key", "")}

@app.post("/api/upload_towers")
def upload_towers(file: UploadFile = File(...)):
    ...
    if 'Signal_dBm' not in df.columns and 'signal_dbm' in df.columns:
        df.rename(columns={'signal_dbm': 'Signal_dBm'}, inplace=True)
    towers = []
    for _, row in df.iterrows():
        try:
            tower = {"mcc": int(row['MCC']), "mnc": int(row['MNC']), "lac": int(row['LAC']), "cellid": int(row['CellID'])}
            if 'Signal_dBm' in df.columns and pd.notna(row['Signal_dBm']):
                tower["signal_dbm"] = float(row['Signal_dBm'])
            towers.append(tower)
        except Exception:
            continue
    if not towers:
        raise HTTPException(400, "No valid tower records found")
    return {"towers": towers, "count": len(towers)}

@app.post("/api/estimate_location")
def estimate_tower_location(data: dict):
    if not LOCATION_ENGINE_AVAILABLE:
        raise HTTPException(503, "Location engine not available. Please ensure location_engine.py is present.")
    towers = data.get("towers")
    if not towers or not isinstance(towers, list):
        raise HTTPException(400, "Missing or invalid 'towers' list")
    unwired_token = data.get("unwired_token", "").strip()
    opencellid_token = data.get("opencellid_token", "").strip()
    if not unwired_token:
        settings = load_settings()
    ...
    try:
        result = estimate_location(tower_list=towers, unwired_token=unwired_token or None, opencellid_token=opencellid_token or None)
        return result
    except Exception as e:
        raise HTTPException(500, f"Location estimation failed: {str(e)}")
